...
class MultiApproval(models.Model):
    ...
    def update_approver(self):
        for rec in self:
            approvers = rec.line_ids.filtered(lambda line: line.state.lower() == 'waiting for approval')
            rec.approver = approvers[0].user_id.id if approvers else False
            if rec.approver:
                template_id = self.env.ref('multi_level_approval.approval_mail_template')
                template_id.send_mail(rec.id, True)
