...
class RequestApproval(models.TransientModel):
    ...
    def default_get(self, fs):
        ...
        obj = self.env[model_name].browse(res_id)
        obj_name = obj.display_name or 'this object'
        title = _('Request approval for {}').format(obj_name)
        obj_url = self._get_obj_url(obj)
        descr = _('''
        Hi,
        </br> </br>
        Please review my request.</br> 
        Click <a target="__blank__" href="{obj_url}"> {obj_name} </a> to view more !
        </br> </br>
        Thanks,
        ''').format(obj_url=obj_url, obj_name=obj_name)
        res.update({
            'name': title,
